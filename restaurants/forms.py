...
class BookingForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if date and time and table:
            ...
            if self.instance.pk:
                existing_bookings = existing_bookings.exclude(pk=self.instance.pk)  # Исключаем текущую бронь при редактировании

            if existing_bookings.exists():
                raise ValidationError('This table is already booked for this date and time.')
...
class RestaurantForm(forms.ModelForm):
    class Meta:
        model = Restaurant
        fields = ['name', 'description', 'address', 'phone_number', 'cuisine', 'price_range']
